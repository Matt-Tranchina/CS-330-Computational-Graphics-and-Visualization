...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// this callback is used to receive mouse scroll movements
...
// When mouse scroll is moved, the speed of movement is adjusted
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// change the view to orthographic
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		// change projection bool value
		bOrthographicProjection = true;

		// change the camera settings to show a front orthographic view
		g_pCamera->Position = glm::vec3(-1.0f, 1.0f, 1.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
		g_pCamera->Front = glm::vec3(0.0f, 0.0f, -1.0f);
	}

	// change the view back to perspective
